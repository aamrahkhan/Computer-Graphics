...
const RGB PointLight::getColor(vec4 & p) {
	...
	double scaling_factor = 1.0 / pow((distance + _deadDistance), _falloff);
	RGB final_val = _illumination * scaling_factor;
	return final_val;
...
Ray PointLight::getShadowRay(vec4 & position, bool & useDist) {
	//YOUR CODE HERE
	Ray shadow_ray(vec3(position), vec3(_pos), 0.01);
	return shadow_ray;
	//IMPLEMENT_ME(__FILE__,__LINE__);
...
Ray DirectionalLight::getShadowRay(vec4 & position, bool & useDist) {
	//YOUR CODE HERE
	Ray shadow_ray(vec3(position), vec3(-_dir), 0.01);
	return shadow_ray;
	//IMPLEMENT_ME(__FILE__,__LINE__);
